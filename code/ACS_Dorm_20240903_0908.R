...
kB <- 0.01 #some reaction rate for m1 + m2 -> B
kC <- 0.01 #some reaction rate for m3 + m4 -> C
kA <- 0.01 #some reaction rate for m5 + m6 -> A
...
for (i in 2:length(vB)){   
  ...
  vA[i] <- vA[i-1] + vAi[i-1]*kAa + (vm5[i-1]*vm6[i-1])*kA + 
    (vC[i-1]*vm5[i-1]*vm6[i-1])*kcA - vA[i-1]*kAi
  ...
  vAi[i] <- vAi[i-1] + vA[i-1]*kAi - vAi[i-1]*kAa
